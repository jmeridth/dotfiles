...
- If the issue-url or issue-id ($1) is provided, add `Relates to $1` as the first line after the commit title to associate the commit to its issue. Use Linear MCP server if available to get issue-url when issue-id is provided
